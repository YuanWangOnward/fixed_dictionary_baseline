function [image,dImage,timeArray]=FBPBlockV3(measure,mask,B,lambda,PM,BPM, tolerance,NIte,iOriginal)
% use  fast iterative soft threshold algorithm (FISTA) to reconstruct image from
% k-space data
% 
% input
...
% dImage: for calculate cost function
% timeArray: record time consumption
%
% version info
% V2
% add step size used when partition the image into blocks
% add image estimation balancing the sparse representation and k-space
% measurement
% V3
% for speed test, remove unnecessary code
%% configure
...
%     iTemp=subjectArray2BlockMatrix(iTemp,blockSize);
    ...
    dImage(nIte)=norm(abs(iTemp(:))-abs(iOriginal(:)))/norm(iOriginal(:));
